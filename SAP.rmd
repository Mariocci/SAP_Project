...
Za svaku varijablu iz dataseta ispitat ćemo jesu li statistički značajne prilikom predikcije za varijablu 'zoo'. Za parametar family postavljen je Gamma jer vrijednosti nisu nužno normalno distribuirane.
...
Rsq
```
Analizom ispisa vidimo koje kategorije su statistički značajne prema p-vrijednostima; statistički najznačajnije su malls, restaurants, pubs/bars, art galleries i cafes.
Varijabla rsq, odnosno pseudo-${R}^2$ danog modela govori da model objašnjava otprilike 51% varijabilnosti u podacima.



Konstruirajmo sada model koji će imati interakcijski član koji uključuje dvije varijable koje su bile među statistički najznačajnijim.
```{r}
...
```
Provedimo nad originalnim i ovim, proširenim modelom test omjera izglednosti te, ukoliko je devijanca proširenog modela značajno manja, prihvatit ćemo ga umjesto originalnog modela.

```{r}
anova(logreg.mdl, logreg.mdl.2, test = "LRT")
```
Konstruirajmo još jedan, smanjeni model, koji neće sadržavati regresore koji nisu bili statistički značajni te ponovno provedimo test omjera izglednosti.
...
```
Konačno, izračunajmo još koeficijent determinacije za prošireni model.
```{r}
Rsq = 1 - logreg.mdl.2$deviance/logreg.mdl.2$null.deviance
Rsq
```
Vidimo kako prošireni model opisuje veću količinu varijabilnosti među podacima nego osnovni model.
